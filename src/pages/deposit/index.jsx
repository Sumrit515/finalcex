...
const index = () => {
  ...
  const [chooseCryptoCurrency, setChooseCryptoCurrency] = useState(
  ...
  const GenerateAddress = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `/api/fetchUserAddress?blockchain=${network}`
      );
      setApiData(data);
      setShowQrCode(true);
        ...
        >
          ...
          {showQrCode && (
            <>
              ...
              <QRCodeReact value={apiData.address} size={150} />
            </>
          )}
        </div>
      </>
      ...
      >
        {/* <DropdownMenu >
            <DropdownMenuTrigger asChild>
                <Button 
                variant="outline"
                onClick={() => console.log(current)}>Clck Me</Button>
            </DropdownMenuTrigger>
